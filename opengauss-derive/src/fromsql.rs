...
pub fn expand_derive_fromsql(input: DeriveInput) -> Result<TokenStream, Error> {
    ...
    let out = quote! {
        impl #impl_generics opengauss_types::FromSql<#lifetime> for #ident #ty_generics #where_clause {
            fn from_sql(_type: &opengauss_types::Type, buf: &#lifetime [u8])
...
fn transparent_body(ident: &Ident, field: &syn::Field) -> TokenStream {
    let ty = &field.ty;
    quote! {
        <#ty as opengauss_types::FromSql>::from_sql(_type, buf).map(#ident)
    }
}
...
fn domain_accepts_body(name: &str, field: &syn::Field) -> TokenStream {
    ...
    quote! {
        if <#ty as opengauss_types::FromSql>::accepts(type_) {
            return true;
        }
...
fn domain_body(ident: &Ident, field: &syn::Field) -> TokenStream {
    let ty = &field.ty;
    quote! {
        <#ty as opengauss_types::FromSql>::from_sql(match *_type.kind() {
            opengauss_types::Kind::Domain(ref _type) => _type,
            _ => _type
        }, buf).map(#ident)
    }
}
...
fn composite_body(ident: &Ident, fields: &[Field]) -> TokenStream {
    ...
    quote! {
        ...
        let mut buf = buf;
        let num_fields = opengauss_types::private::read_be_i32(&mut buf)?;
        ...
        for field in fields {
            let oid = opengauss_types::private::read_be_i32(&mut buf)? as u32;
                ...
                #(
                    #field_names => {
                        #temp_vars = std::option::Option::Some(
                            opengauss_types::private::read_value(field.type_(), &mut buf)?);
                    }
                )*
                _ => unreachable!(),
            }
