...
use tokio_opengauss::{Error, Row, SimpleQueryMessage, Socket};
...
impl Client {
    pub(crate) fn new(connection: Connection, client: tokio_opengauss::Client) -> Client {
    ...
    ///
    /// ```no_run
    /// use opengauss::{Client, NoTls, types::Type};
    ///
    /// # fn main() -> Result<(), opengauss::Error> {
    /// let mut client = Client::connect("host=localhost user=postgres", NoTls)?;
    ///
    ...
    /// ```no_run
    /// # use opengauss::{Client, NoTls};
    /// use opengauss::types::{ToSql, Type};
    /// use fallible_iterator::FallibleIterator;
    /// # fn main() -> Result<(), opengauss::Error> {
    /// # let mut client = Client::connect("host=localhost user=postgres", NoTls)?;
    ///
