...
use tokio::runtime;
#[doc(inline)]
pub use tokio_opengauss::config::{
    ChannelBinding, Host, LoadBalanceHosts, SslMode, SslNegotiation, TargetSessionAttrs,
};
use tokio_opengauss::error::DbError;
use tokio_opengauss::tls::{MakeTlsConnect, TlsConnect};
use tokio_opengauss::{Error, Socket};
...
impl Config {
    ...
    pub fn new() -> Config {
        tokio_opengauss::Config::new().into()
    }
    ...
    ///
    /// [`AsyncMessage::Notice`]: tokio_opengauss::AsyncMessage::Notice
...
impl FromStr for Config {
    ...
    fn from_str(s: &str) -> Result<Config, Error> {
        s.parse::<tokio_opengauss::Config>().map(Config::from)
    }
}

impl From<tokio_opengauss::Config> for Config {
    fn from(config: tokio_opengauss::Config) -> Config {
        Config {
            config,
